# Planification voyage Japon 2026

Ce dossier centralise la planification du voyage au Japon du 04 au 27 juillet 2026. Il sert de base commune pour toutes les conversations Codex liées au voyage, afin de reprendre le contexte rapidement sans repartir de zéro.

## Reprise du projet

Au début de chaque conversation sur ce projet, lire dans cet ordre :

1. `AGENTS.md`
2. `README.md`
3. `CONTEXTE_VOYAGE.md`
4. `ITINERAIRE.md`
5. `DECISIONS.md`
6. `TODO.md`

Lire ensuite les fichiers spécialisés selon le sujet :

- `HOTELS.md` pour les recherches et décisions d'hébergement.
- `BUDGET.md` pour les estimations et plafonds.
- `COUPE_DU_MONDE_PORTUGAL.md` pour les matchs potentiels du Portugal.
- `SOURCES.md` pour retrouver les liens et vérifier les informations.

## Règles de workflow

- Ne considérer comme validé que ce qui est inscrit dans `DECISIONS.md`.
- Après une recherche ou une décision validée par Kev, proposer une mise à jour des fichiers concernés.
- Ne jamais créer, modifier, supprimer, renommer ou déplacer un fichier sans validation explicite.
- Après validation, limiter les modifications exactement au périmètre accepté.
- Les recherches, lectures, analyses et propositions ne demandent pas de validation.

## Rôle des fichiers

| Fichier | Rôle |
|---|---|
| `CONTEXTE_VOYAGE.md` | Contexte général, dates, voyageurs, contraintes, préférences |
| `ITINERAIRE.md` | Planning jour par jour avec confirmé, à confirmer et idées |
| `HOTELS.md` | Zones recommandées, plafonds, shortlist et décision finale |
| `BUDGET.md` | Estimation globale et par catégorie |
| `DECISIONS.md` | Source de vérité des décisions validées |
| `TODO.md` | Prochaines actions à faire |
| `COUPE_DU_MONDE_PORTUGAL.md` | Scénarios Coupe du monde 2026 pour le Portugal |
| `SOURCES.md` | Liens utiles, sources prix, notes de fiabilité |

